...
import { botTurn, discard, newGame, players, tileLabel, seatWind, claimOptions, claimKey, resolveClaims, concealedKongs, declareConcealedKong } from './engine'
...
export default function GamePage() {
  ...
  const playing = !opening && game.result === 'playing', yourTurn = playing && !game.pending && game.turn === 0 && ready && !error
  ...
  return <div className="game-page">
    ...
    <main className="game-main" inert={opening || rules || restart || !playing}>
      ...
      <section className="player-area"><div className="turn-line"><div><span className={`turn-dot ${yourTurn ? 'is-you' : ''}`}/><strong>{!playing ? 'Round complete' : game.pending ? 'Claim window' : yourTurn ? 'Your turn' : `${players[game.turn]}'s turn`}</strong><span className="your-seat">YOU · {seatWind(0, game.dealer).toUpperCase()}{game.dealer === 0 ? ' · DEALER' : ''}</span></div><p aria-live="polite">{game.message}</p></div>
        {game.pending && playing && <div className="claim-panel" aria-label="Discard claim choices">
          <img className="claim-discard" src={faces[game.pending.tile.type]} alt={tileLabel(game.pending.tile.type)}/>
          <div className="claim-details"><strong>{players[game.pending.seat]} discarded {tileLabel(game.pending.tile.type)}</strong><p>{options.length ? 'Claim this tile or pass. The table waits for your decision.' : 'You have no legal claim. Checking the other players…'}</p>
            <div className="claim-actions">{options.map(option => {
              const types = [...option.tileIds.map(id => game.hands[0].find(t => t.id === id)!.type), game.pending!.tile.type].sort((a,b) => a-b)
              return <button key={claimKey(option)} disabled={!ready || !!error} onClick={() => { setGame(current => resolveClaims(current, option)); setSelected(null) }} aria-label={option.kind === 'win' ? 'Claim Mahjong win' : `Claim ${option.kind}: ${types.map(tileLabel).join(', ')}`}>
                <strong>{option.kind === 'win' ? 'Mahjong!' : option.kind === 'chi' ? 'Chi (Che)' : option.kind === 'pong' ? 'Pong' : 'Kong'}</strong>
                {option.kind !== 'win' && <span className="claim-preview">{types.map((type,i) => <img key={i} src={faces[type]} alt={tileLabel(type)}/>)}</span>}
              </button>
            })}{options.length > 0 && <button className="pass-claim" disabled={!ready || !!error} onClick={() => { setGame(current => resolveClaims(current, null)); setSelected(null) }}>Pass →</button>}</div>
          </div>
        </div>}
